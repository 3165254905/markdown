...
2.  什么是内部比较器与外部比较器？
    - $\color{red}{内部比较器是 comparable 接口}$
    - $\color{red}{外部比较器是 comparator 接口}$
3.  <span id="jump1">Comparable 内部比较器（可能不考）</span>
    ...
    2. String 类型比较
       ...
       $\color{red}{a.comparaTo(b) a 小，返回-1 ，a 大返回 1。}$
    ...
    3. double 类型比较
       ...
       当然如果看不懂这个也没关系，只需要记住 $\color{red}{double 类型比较时要转化为 Double 类型}$就可以了。
    ...
    $\color{red}{在写 Student 类时，要在 public class Student 后加上 implements Comparable，其中 Comparable 注意拼写！！！C 为大写！！！！}$
